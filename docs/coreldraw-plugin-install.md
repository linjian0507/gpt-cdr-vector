...
参照图旁边的 `先识图生成 1:1 提示词` 开关用于两段式生成：当已经选择图片、使用 CDR 选中对象或粘贴截图后，面板会先调用一次视觉模型，把图片转换成尽量 1:1 还原的 SVG 生成提示词，并自动写回 `图形描述`；随后再用这段提示词继续生成 SVG。新版两段式会优先做文字 OCR、版式地图、元素清单、颜色清单和禁止改动项，再要求第二步严格按参照图还原。该模式更适合还原现有截图、包装稿、图标或版式，但会多消耗一次模型请求时间和费用。

还原含中文标题/正文的海报时，建议保留 `先识图生成 1:1 提示词`，并允许使用 SVG 文字；这样模型会优先生成可编辑 `<text>`，避免把中文标题画成不可读的抽象字形。需要注意：这仍然是模型重建 SVG，不是专业位图自动描摹，复杂插画和字体效果只能尽量接近。

`混合还原（底图+可编辑层）` 是更稳的参照图工作流。启用后，生成文件会把压缩后的参照图嵌入为底部位图层，再让模型只生成可编辑覆盖层，例如 OCR 文字、边框、简单色块、装饰线和粗略人物/物体引导层。这样 CDR 文件不是纯矢量，但更适合实际修稿：原图保真在底层，能编辑的文字和简单版式在上层。

面板发送 PNG/JPG/JPEG 参照图前会自动压缩到适合视觉模型识别的大小，减少截图过大导致中转接口在发送阶段断开的概率。普通纯 SVG 流程不会把参照位图嵌入结果；`混合还原（底图+可编辑层）` 会把压缩参照图作为底部位图层嵌入。
...
如果生成时报 `发送时发生错误` 或 `接口请求发送失败`，说明连接在上传请求体时就被网关关闭，常见原因是截图参照图过大、网络代理限制或中转接口限制。新版 Addon 已关闭 `Expect: 100-continue` 和长连接，并会自动压缩 PNG/JPG/JPEG 参照图；仍失败时请重试一次，或换更小的参照图/更稳定的接口。
...
- `先识图生成 1:1 提示词`：有参照图时先调用模型把图片整理成可还原的 SVG 生成提示词，自动回填到 `图形描述`，再继续生成 SVG；新版会强制列出 OCR 文字、版式地图、元素清单和禁止改动项，这会多一次接口请求。
- `混合还原（底图+可编辑层）`：有参照图时生成一个混合 SVG，底层保留参照位图，上层生成可编辑文字、边框和简单矢量元素；适合复杂海报修稿，不适合要求全图纯矢量交付的场景。
